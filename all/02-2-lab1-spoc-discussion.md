...
> 最开始先实现一个可以显示字符串的bootloader，之后再实现寄存器数值的初始化。
> 之后再完成将CPU从实模式切换到保护模式的功能。
> 再实现实现读取ELF格式文件的功能。
> 最后就可以切换到kernel代码进行执行了。


如何能获取一个系统调用的调用次数信息？如何可以获取所有系统调用的调用次数信息？请简要说明可能的思路。(spoc)
- [x]  

> 在应用程序中植入一个计数器，每次执行系统调用的时候均会将计数器的数值加一。同时维护一个调用日志，每次执行系统调用的时候，都会记录该次调用的详细情况，最后，设置一个开关，若用户需要获取系统调用信息则会将统计的数据以及日志信息输出。
